...
/**
 * Legacy eager combo transform retained only for the fork's rollback baseline.
 * The new recovery path does not call it: normal traffic stays unchanged and
 * completed-turn thinking is omitted only after Anthropic's exact signature 400.
 */
...
export async function executeWithAnthropicThinkingSignatureRecovery<T>(args: {
  ...
  const error = await args.getError(first);
  if (
    !error ||
